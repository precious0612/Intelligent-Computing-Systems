...
class VGG19:
    ...
    def conv2d(self, x, weights, bias, padding='same'):
        ...
        for i in range(width):
            for j in range(height):
                ...
                output[:, i, j] = blas.dgemv(1.0, kernel.flatten().reshape(output_channel, -1), array_comp).reshape(output_channel, ) + bias # replaced np.dot with np.matmul
    ...
    def dense(self, x, weights, bias, activation='relu'):
        x = x.reshape(-1)
        out = blas.dgemv(1, weights.T, x) + bias  # replaced np.dot with np.matmul
        if activation == 'relu':
            out[out < 0] = 0
